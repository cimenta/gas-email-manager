...
const TRANSPORT_TICKETS_ACTION_CONFIG = {
  ...
  // insertPdfIntoEvent } — the IDENTICAL shape as
  // TICKETING_PORTALS_ACTION_CONFIG.ticketingPortals (src/07-action-cfg-
  // ticketing-portals.js), validated with the SAME shared
  // isValidTicketingPortalsShape (see src/01-setup.js's own JSDoc on that
  // function for why it is shared, not cloned). Each entry identifies one
  // supported transport carrier by its confirmation email's sender address
  // (case-insensitive match via resolveTransportSender in the sibling
  // action file), the calendar its events should be created on, and
  // whether the ticket PDF should be archived + attached to the created
  // Calendar event.
  //
  // The shipped default seeds ONE entry: jizdenky@regiojet.cz (D-05,
  // extensible sender registry — future carriers are added the same way).
  ...
  // {identifyingEmail, calendarId, insertPdfIntoEvent} objects).
  ...
  // JSON-typed setting):
  // [{"identifyingEmail":"jizdenky@regiojet.cz","calendarId":"abc123@group.calendar.google.com","insertPdfIntoEvent":true}]
